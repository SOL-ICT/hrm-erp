...
const AuthDebugger = () => {
  ...
  const testAPI = async () => {
    try {
      ...
      setTestResult(`Success (${duration}ms): ${JSON.stringify(response, null, 2)}`);
    } catch (error) {
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-black/90 text-green-400 p-4 rounded-lg text-xs font-mono z-50 max-w-sm">
      <h3 className="text-white font-bold mb-2">🔐 Auth Debug {isOptimized && "⚡"}</h3>
      <div className="space-y-1 mb-3">
        ...
        {tokenStatus && (
          <>
            <div>Token: {tokenStatus.hasToken ? "✅ Present" : "❌ Missing"}</div>
            <div>Token Length: {tokenStatus.tokenLength || 0}</div>
            {typeof window !== "undefined" && localStorage.getItem("auth_token") && (
              <div className="text-xs text-gray-300 break-all">
                Bearer: {localStorage.getItem("auth_token")?.substring(0, 20)}...
              </div>
            )}
          </>
        )}
