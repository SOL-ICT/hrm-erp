...
const ClientServiceLocation = ({ currentTheme, preferences, onBack }) => {
  ...
  const loadLocations = async (page = 1) => {
    ...
    try {
      ...
      if (viewMode === "grouped") {
        ...
        response = await sanctumRequest(`http://localhost:8000/api/service-locations/grouped-by-client?${params}`);
        ...
        response = await sanctumRequest(`http://localhost:8000/api/service-locations?${params}`);
  ...
  const handleCreateLocation = async (locationData) => {
    try {
      const response = await sanctumRequest('http://localhost:8000/api/service-locations', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateLocation = async (id, locationData) => {
    try {
      const response = await sanctumRequest(`http://localhost:8000/api/service-locations/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const LocationMaster = () => {
    return (
      <div className="space-y-6">
        ...
        {/* Filters */}
        <div className={`${currentTheme.cardBg} rounded-lg border ${currentTheme.border} p-4`}>
  ...
  const FlatLocationView = () => {
    return (
      <div className={`${currentTheme.cardBg} rounded-lg border ${currentTheme.border} overflow-hidden`}>
              ...
              {locations.map((location) => (
                <tr key={location.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4">
                    <div className={`text-sm ${currentTheme.textPrimary} max-w-xs truncate`}>
  ...
  const GroupedLocationView = () => {
    return (
      <div className="space-y-4">
        {Object.entries(groupedLocations).map(([clientName, clientLocations]) => (
          <div key={clientName} className={`${currentTheme.cardBg} rounded-lg border ${currentTheme.border}`}>
            <div
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`text-lg font-medium ${currentTheme.textPrimary}`}>
                      {clientName}
                    </h3>
                    <p className={`text-sm ${currentTheme.textSecondary}`}>
                      {clientLocations.stats?.total_locations || 0} location{(clientLocations.stats?.total_locations || 0) !== 1 ? 's' : ''}
                    </p>
                  </div>
                </div>
            ...
            {expandedGroups.has(clientName) && (
              <div className="border-t border-gray-200 dark:border-gray-700">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                  {clientLocations.locations && clientLocations.locations.map((location) => (
