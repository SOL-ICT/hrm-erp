...
const Boarding = ({ currentTheme, preferences, onBack }) => {
  ...
  const loadTickets = async (clientId) => {
    try {
      setLoading(true);
      const response = await boardingAPI.getTickets(clientId);
      if (response.success) {
  ...
  const loadCandidatesForTab = async () => {
    try {
      setLoading(true);
      let response;
      if (activeTab === 'issue-offer') {
        response = await boardingAPI.getCandidatesForOffer(selectedTicket);
      } else {
        response = await boardingAPI.getCandidatesForBoarding(selectedTicket);
      }
  ...
  const loadJobStructureAndGrades = async () => {
    const ticket = tickets.find(t => t.id == selectedTicket);
    if (ticket) {
      try {
        const response = await boardingAPI.getPayGrades(ticket.job_structure_id);
        if (response.success) {
  ...
  const handleSendOffer = async (candidateId, payGradeId) => {
    try {
      setLoading(true);
      const ticket = tickets.find(t => t.id == selectedTicket);
      const offerData = {
      ...
      if (response.success) {
        ...
        loadCandidatesForTab(); // Refresh candidates list
      } else {
  ...
  const handleBoardCandidate = async (candidateId) => {
    try {
      setLoading(true);
      const candidate = candidates.find(c => c.id === candidateId);
      const response = await boardingAPI.boardCandidates({ 
        candidates: [candidate.boarding_request_id] 
      });

      if (response.success) {
        alert('Candidate boarded successfully!');
        loadCandidatesForTab(); // Refresh candidates list
      } else {
        alert('Failed to board candidate. Please try again.');
      }
    } catch (error) {
      console.error('Failed to board candidate:', error);
      alert('Failed to board candidate. Please try again.');
    } finally {
      setLoading(false);
    }
