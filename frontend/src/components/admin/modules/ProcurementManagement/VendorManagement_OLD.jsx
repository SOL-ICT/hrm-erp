...
export default function VendorManagement({ currentTheme, preferences, onBack }) {
  const [activeTab, setActiveTab] = useState('vendors'); // 'vendors' or 'logs'
  ...
  const filteredLogs = procurementLogs.filter(log =>
    ...
    log.inventory_item?.item_name?.toLowerCase().includes(logSearchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl backdrop-blur-md shadow-lg overflow-hidden`}>
        ...
        <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-shadow`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs ${currentTheme.textSecondary} uppercase tracking-wide`}>Total Vendors</p>
        ...
        <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-shadow`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs ${currentTheme.textSecondary} uppercase tracking-wide`}>Active Vendors</p>
        ...
        <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-shadow`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs ${currentTheme.textSecondary} uppercase tracking-wide`}>Total Procurements</p>
              <p className={`text-2xl font-bold ${currentTheme.textPrimary} mt-1`}>
        ...
        <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-shadow`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs ${currentTheme.textSecondary} uppercase tracking-wide`}>This Month</p>
              <p className={`text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1`}>
                {procurementLogs.filter(log => {
      ...
      {/* Modern Tabs */}
      <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl backdrop-blur-md shadow-lg overflow-hidden`}>
        <div className="flex items-center gap-1 p-2 bg-gray-50 dark:bg-slate-800/50">
          <button
            onClick={() => setActiveTab('vendors')}
            className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
              activeTab === 'vendors'
                ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm'
                : `${currentTheme.textSecondary} hover:bg-white/50 dark:hover:bg-slate-700/50`
            }`}
          >
        ...
        <div className="p-4 border-t dark:border-slate-700">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={activeTab === 'vendors' ? 'Search vendors by name, code, category...' : 'Search logs by vendor, invoice, item...'}
              ...
              className={`w-full pl-10 pr-4 py-2.5 ${currentTheme.inputBg} ${currentTheme.border} ${currentTheme.textPrimary} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm`}
            />
          </div>
        </div>
      </div>
      ...
      {/* Tab Content */}
      {activeTab === 'vendors' ? (
        /* VENDOR DIRECTORY TAB - Compact Grid */
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {vendorsLoading ? (
            <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-8 text-center col-span-full backdrop-blur-md shadow-sm`}>
            ...
            filteredVendors.map((vendor) => (
              <div 
                key={vendor.id} 
                className={`${currentTheme.cardBg} ${currentTheme.border} rounded-lg p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-all duration-200 group`}
              >
        ...
        /* PROCUREMENT LOGGING TAB - Compact Table Layout */
        <div className="space-y-3">
          {logsLoading ? (
            <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-xl p-8 text-center backdrop-blur-md shadow-sm`}>
              <div className="flex flex-col items-center gap-2">
            ...
            filteredLogs.map((log) => (
              <div 
                key={log.id} 
                className={`${currentTheme.cardBg} ${currentTheme.border} rounded-lg p-4 backdrop-blur-md shadow-sm hover:shadow-md transition-all duration-200`}
              >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className={`font-semibold ${currentTheme.textPrimary} text-sm truncate`}>
                          {log.inventory_item?.item_name || 'Unknown Item'}
                        </h3>
      ...
      {((activeTab === 'vendors' && vendorPagination.totalPages > 1) || 
        (activeTab === 'logs' && logPagination.totalPages > 1)) && (
        <div className={`${currentTheme.cardBg} ${currentTheme.border} rounded-lg p-3 flex justify-between items-center backdrop-blur-md shadow-sm`}>
          <div className={`text-xs ${currentTheme.textSecondary}`}>
            Showing <span className="font-semibold">{activeTab === 'vendors' ? filteredVendors.length : filteredLogs.length}</span> of{' '}
            <span className="font-semibold">{activeTab === 'vendors' ? vendorPagination.total : logPagination.total}</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => activeTab === 'vendors' 
                ? fetchVendors(vendorPagination.currentPage - 1) 
                : fetchProcurementLogs(logPagination.currentPage - 1)}
              disabled={activeTab === 'vendors' 
                ? vendorPagination.currentPage <= 1 
                : logPagination.currentPage <= 1}
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                (activeTab === 'vendors' ? vendorPagination.currentPage <= 1 : logPagination.currentPage <= 1)
            ...
            <span className={`px-3 py-1.5 text-sm ${currentTheme.textPrimary} font-medium`}>
              {activeTab === 'vendors' ? vendorPagination.currentPage : logPagination.currentPage} / {activeTab === 'vendors' ? vendorPagination.totalPages : logPagination.totalPages}
            </span>
            <button
              onClick={() => activeTab === 'vendors' 
                ? fetchVendors(vendorPagination.currentPage + 1) 
